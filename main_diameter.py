...
def run_simulator(simulation_time=10000, eps=0.7, max_radius=10000, window_size=100, input_points=None):
    ...
    for i in range(simulation_time):
        ...
        expected_center_old, expected_center_new, expected_diameter, ds_result = d_simulator.execute_one_cycle(new_point)
        if expected_diameter == 0:
            print("Diameter is 0,", ds_result.diameter)
            continue

        is_correct = ds_result.alpha <= expected_diameter <= ds_result.alpha * (1+eps)
        print(d_simulator.current_time, is_correct, ds_result.alpha, expected_diameter, ds_result.diameter)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        run_simulator(simulation_time=10000, eps=0.5, max_radius=1000, window_size=3)
    else:
